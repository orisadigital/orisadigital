...
import SoftwarePluginSheet from "@/components/renewals/SoftwarePluginSheet";
...
export default function SoftwarePlugin() {
  ...
  const load = async () => {
    try {
      const data = await base44.entities.SoftwarePlugin.list("-renewal_date");
      setItems(data);
    } catch (e) {
      console.error("Failed to load software/plugins", e);
  ...
  const handleDelete = async (id) => {
    try {
      await base44.entities.SoftwarePlugin.delete(id);
  ...
  const handleSave = async (payload) => {
    try {
      if (editing) {
        ...
        const updated = await base44.entities.SoftwarePlugin.update(id, rest);
        setItems((prev) => prev.map((i) => (i.id === id ? updated : i)));
        toast.success("Updated");
        setEditing(null);
      } else {
        const created = await base44.entities.SoftwarePlugin.create(payload);
  ...
  return (
    <div className="space-y-4">
      <LoadErrorBanner label="software & plugins" error={loadError} />
              ...
              items.map((i) => {
                ...
                return (
                  <TableRow key={i.id} className="hover:bg-slate-50/50">
                    ...
                    <TableCell className="px-4 text-slate-600 capitalize">{i.category}</TableCell>
      ...
      <SoftwarePluginSheet record={editing} open={Boolean(editing)} onSave={handleSave} onClose={() => setEditing(null)} />
      <SoftwarePluginSheet record={null} open={adding} onSave={handleSave} onClose={() => setAdding(false)} />
    </div>
  );
}
