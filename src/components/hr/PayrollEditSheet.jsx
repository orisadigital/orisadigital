...
export default function PayrollEditSheet({ record, onUpdate, onClose }) {
  const [form, setForm] = useState({});
  const [isOtherName, setIsOtherName] = useState(false);
  const [isOtherPosition, setIsOtherPosition] = useState(false);
  const projects = useProjects(Boolean(record));

  useEffect(() => {
    if (record) {
      const name = record.employee_name || "";
      setIsOtherName(Boolean(name) && !EMPLOYEE_NAMES.includes(name));
      const position = record.employee_position || "";
      setIsOtherPosition(Boolean(position) && !POSITIONS.includes(position));
      setForm({
  ...
  return (
    <Sheet open={Boolean(record)} onOpenChange={(open) => !open && onClose?.()}>
      <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Edit Payroll Record</SheetTitle>
        </SheetHeader>
              ...
              <div className="col-span-2">
                <Field label="Payment Type">
                  <select value={form.payment_type} onChange={handlePaymentTypeChange} className={selectCls}>
                    ...
                    {form.payment_type && !PAYMENT_TYPES.includes(form.payment_type) && (
                      <option value={form.payment_type}>{form.payment_type}</option>
                    )}
                  </select>
                </Field>
              </div>
          ...
          <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Amount (RM)">
                <Input type="number" min="0" step="0.01" value={form.basic_salary} onChange={setNum("basic_salary")} className={inputCls} />
              </Field>
