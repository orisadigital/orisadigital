...
import { Save, Boxes, Lock, FileText } from "lucide-react";
...
export default function SubscriptionSheet({ record, open, onSave, onClose }) {
  ...
  return (
    <Sheet open={open} onOpenChange={(o) => !o && onClose?.()}>
      <SheetContent side="right" className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{record ? "Edit" : "Add"} Subscription</SheetTitle>
        </SheetHeader>

        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          <section className="space-y-3">
            <SectionHeader icon={Boxes} title="Subscription Details" />
            <div className="grid grid-cols-2 gap-3">
              <Field label="Name" required>
                <Input value={form.name} onChange={set("name")} className={inputCls} placeholder="Google Workspace" />
              </Field>
              <Field label="Category">
                <Input value={form.category} onChange={set("category")} className={inputCls} placeholder="SaaS / Cloud" />
              </Field>
              <Field label="Provider">
                <Input value={form.provider} onChange={set("provider")} className={inputCls} placeholder="Google" />
              </Field>
